...
- [ ] FE-02: API functions — Add to `lib/api.ts`: `listFeedback(params?)`, `getFeedback(id)`, `createFeedback(data)`, `updateFeedback(id, data)`, `deleteFeedback(id)`
- [ ] FE-03: AppHeader component — App title + "Log Feedback" nav button (ui-spec § AppHeader)
- [ ] FE-04: FilterBar component — Search input + theme dropdown + status dropdown + clear filters (ui-spec § FilterBar)
...
- [ ] FE-06: FeedbackForm component — Shared form for create/edit modes with validation; delete button in edit mode (ui-spec § FeedbackForm)
- [ ] FE-07: Feedback List page — Route /feedback composing AppHeader + FilterBar + FeedbackTable (ui-spec § Feedback List)
- [ ] FE-08: Log Feedback page — Route /feedback/new with FormHeader + FeedbackForm in create mode (ui-spec § Log Feedback)
- [ ] FE-09: Edit Feedback page — Route /feedback/[id] with FormHeader + FeedbackForm in edit mode + NotFoundState (ui-spec § Edit Feedback)
- [ ] FE-10: Layout + routing — Root layout, redirect / → /feedback, catch-all 404 → /feedback
